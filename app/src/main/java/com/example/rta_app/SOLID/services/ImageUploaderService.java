    ...
    public void loadImagem() {
        if (currentUser != null) {
            ...
            gsReference.getDownloadUrl().addOnSuccessListener(uri -> {
                Picasso.get().load(uri).into(((ProfileActivity) context).binding.UserImagenView);
            }).addOnFailureListener(exception -> {
                ((ProfileActivity) context).binding.UserImagenView.setImageResource(R.drawable.baseimageforuser);
            });
        }
    }
